...
def count_pairs(
    ...
    """
    Counts the number of occurrences of each pair of tokens in the tokens list.
    """
    pair_counts = defaultdict(int)
    for seq in tokens:
        for i in range(len(seq) - 1):
            # Skip pairs that contain special tokens
            if seq[i] in special_tokens or seq[i + 1] in special_tokens:
                continue
...
def merge_naive(
    ...
    while len(vocab) < vocab_size:
        ...
        pair_counts = count_pairs(tokens, special_tokens)
        if not pair_counts:
            break
        ...
        for seq in tokens:
            ...
            while i < len(seq):
                if i < len(seq) - 1 and (seq[i], seq[i + 1]) == best_pair:
                    ...
                    i += 2  # skip the next token, as we've merged the pair
                else:
...
def merge_with_heap(
    ...
    while len(vocab) < vocab_size and heap:
        ...
        # Gather all ties with the same frequency
        while heap and -heap[0][0] == freq:
            _, _, next_pair = heapq.heappop(heap)
            if pair_counts.get(next_pair, 0) == freq:
                ties.append(next_pair)

        # Pick lexicographically smallest decoded pair
        pair = min(
            ties,
            key=lambda p: (
                vocab[p[0]].decode('utf-8', errors='replace'),
        ...
        # Push back the losers into the heap
        for loser in ties:
            if loser != pair:
                loser_key = tiebreak_key(loser[0], loser[1], vocab)
                heapq.heappush(heap, (-freq, loser_key, loser))
